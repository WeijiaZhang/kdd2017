...
def trivial_feature_v2():
    '''定义为tollgate的特征，也就是全局计算最大、最小和平均流量'''
    r = Repo()
    df = r(volume_grouped, phase='train', name='train')
    df['tmp_time'] = df.time.str[11:]
    gp = df.groupby(['tollgate_id', 'direction', 'tmp_time']).agg({'volume': [np.max,
                                                                              np.min, np.mean]}).reset_index()
    gp.columns = ['tollgate_id', 'direction', 'tmp_time',
                  'max_vol', 'min_vol', 'avg_vol']
    df = r(volume_grouped, phase='train', name='train')
    df['tmp_time'] = df.time.str[11:]
    df = df.merge(gp, on=['tollgate_id', 'direction', 'tmp_time'], how='left')
    df = df.drop('tmp_time', axis='columns')
    df['hour'] = df.time.apply(lambda t: round(
        (int(t[11:13]) * 60 + int(t[14:16])) / 60) % 24)
    df = df.reindex_axis(['tollgate_id', 'direction', 'time', 'hour',
                          'max_vol', 'min_vol', 'avg_vol', 'volume'], axis='columns')
    return df
