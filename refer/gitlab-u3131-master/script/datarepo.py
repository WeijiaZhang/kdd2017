...
class Repo:
    ...
    '''
    ins = None  # 单例模式下用于保存那个唯一的对象

    def __init__(self, baseDir='../temp/repo1'):
        '''baseDir是缓存数据的文件夹'''
        #__init__每个获取对象的时候都会调用，用hasattr判断它是否已被初始化过，
        # 以免重复初始化
    ...
    def __new__(cls, *args, **kwargs):
        ...
        if not cls.ins:
            cls.ins = super(Repo, cls).__new__(
                cls, *args, **kwargs)
        return cls.ins

    def __call__(self, func, name=None, *args, **kwargs):
    ...
    def saveResult(self, result, name='none'):
        ...
        stamp = time.strftime('%m%d_%H_%M', time.localtime(time.time()))
